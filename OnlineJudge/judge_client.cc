...
#define REG_ARG1 rsi
//c & c++
int LANG_CV[256] = {0,1,2,3,4,5,8,9,10,11,12,20,21,59,63,89,158,231,240,272,511, SYS_time, SYS_read, SYS_uname, SYS_write, SYS_open,
		SYS_close, SYS_execve, SYS_access, SYS_brk, SYS_munmap, SYS_mprotect,
...
int compile(int lang, char* submit_id) {
	...
	if (lang == 1) {//c
		sprintf(compile_str, "gcc %s.c -o %s", submit_id, submit_id);
	}
	else if (lang == 2) {//c++
